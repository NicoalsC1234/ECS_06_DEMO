...
def system_enemy_hunter_state(world: esper.World, player_entity: int, hunter_info: dict, enemy:int):
    ...
    for _, (c_st, c_a, c_t, c_v) in components:
        if c_st.state == HunterState.IDLE:
            _do_enemy_hunter_idle(c_st, c_a, c_t, c_v, pl_t, hunter_info,enemy)
        elif c_st.state == HunterState.CHASE:
            _do_enemy_hunter_chase(c_st, c_a, c_t, c_v, pl_t, hunter_info,enemy)
...
def _do_enemy_hunter_idle(c_st: CEnemyHunterState, c_a: CAnimation, c_t: CTransform,
                          c_v: CVelocity, pl_t: CTransform, hunter_info: dict,enemy:int):
    ...
    if enemy==2:
        if dist_to_player/2 < hunter_info["distance_start_chase"]:
            c_st.state = HunterState.CHASE
    elif enemy ==3:
        if dist_to_player/3 < hunter_info["distance_start_chase"]:
            c_st.state = HunterState.CHASE
    else:
...
def _do_enemy_hunter_chase(c_st: CEnemyHunterState, c_a: CAnimation, c_t: CTransform,
    ...
    if enemy==2:
        ...
        c_v.vel = ((pl_t.pos - c_t.pos).normalize() * hunter_info["velocity_chase"])/2
        ...
        set_animation(c_a, "MOVE")
        c_v.vel = (pl_t.pos - c_t.pos).normalize() * hunter_info["velocity_chase"]/3
        dist_to_origin = c_st.start_pos.distance_to(c_t.pos)
        if dist_to_origin >= hunter_info["distance_start_return"]:
            c_st.state = HunterState.RETURN    
    else:
